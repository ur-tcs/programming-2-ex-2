...
between the root and a leaf. The example tree on the git repository has a depth of 2.*/
...
in pre-order. This means that the value of each node must be printed before its children,
and then the values of the left subtree must be printed before the values of the right subtree.

For the example tree in the README file, IntBranch(1, IntBranch(2, IntEmptyTree(), IntEmptyTree()), IntEmptyTree()), the output should be:
