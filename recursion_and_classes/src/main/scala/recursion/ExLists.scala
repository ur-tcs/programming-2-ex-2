...
So ask yourself: how would you count how many nested dolls a large Matryoshka doll contains?
Can you translate this process into code?*/
...
If min is called on an empty list, it should throw an IllegalArgumentException using throw IllegalArgumentException("Empty list!").*/
...
/*Implement a function that appends the reversed version of an IntList to another.
For example, calling reverseAppend with the lists 1, 2, 3 and 7, 8, 9 should produce 3, 2, 1, 7, 8, 9*/
...
/*Implement a function that creates a new list containing the same elements as its input list,
except that it discards every number starting from the first negative number.
...
//Implement a function that creates a copy of an IntList with the last element removed. This functions is sometimes called butlast:
...
//Implement a function, that evaluates an expression in Polish Notation, see the README file for more information about Polish notation.
val Add = -1
val Multiply = -2
